...
# ---------------------------------------------------------------------------
# Peer map — for each ticker, list its closest sector/business peers.
# Used to detect divergences (is the stock moving with its peers, or alone?)
# Also includes the sector ETF as a benchmark.
# ---------------------------------------------------------------------------
PEERS: dict[str, list[str]] = {
    # Big banks
    "BAC":  ["JPM", "WFC", "C", "USB", "PNC", "XLF"],
    "JPM":  ["BAC", "WFC", "C", "GS", "MS", "XLF"],
    "WFC":  ["BAC", "JPM", "C", "USB", "PNC", "XLF"],
    "C":    ["BAC", "JPM", "WFC", "GS", "MS", "XLF"],
    # Investment banks / brokers
    "GS":   ["MS", "JPM", "C", "SCHW", "XLF"],
    "MS":   ["GS", "JPM", "C", "SCHW", "XLF"],
    "SCHW": ["IBKR", "GS", "MS", "XLF"],
    # Mega-cap tech
    "AAPL": ["MSFT", "GOOGL", "META", "AMZN", "XLK", "QQQ"],
    "MSFT": ["AAPL", "GOOGL", "ORCL", "CRM", "XLK", "QQQ"],
    "GOOGL":["META", "MSFT", "AAPL", "AMZN", "XLK", "QQQ"],
    "AMZN": ["GOOGL", "META", "MSFT", "AAPL", "XLY", "QQQ"],
    "META": ["GOOGL", "MSFT", "AAPL", "AMZN", "XLC", "QQQ"],
    "NVDA": ["AMD", "AVGO", "INTC", "TSM", "SMH", "QQQ"],
    # Beverages / staples
    "KO":   ["PEP", "MNST", "KDP", "XLP"],
    "PEP":  ["KO", "MNST", "KDP", "XLP"],
    "PG":   ["CL", "KMB", "CHD", "XLP"],
    ...
    "WMT":  ["COST", "TGT", "DG", "XLP"],
    "COST": ["WMT", "TGT", "BJ", "XLP"],
    "MCD":  ["SBUX", "YUM", "QSR", "XLY"],
    # Energy
    "XOM":  ["CVX", "COP", "EOG", "XLE"],
    "CVX":  ["XOM", "COP", "EOG", "XLE"],
    "COP":  ["XOM", "CVX", "EOG", "OXY", "XLE"],
    # Healthcare
    "JNJ":  ["PFE", "MRK", "ABBV", "LLY", "XLV"],
    "PFE":  ["JNJ", "MRK", "ABBV", "BMY", "XLV"],
    "UNH":  ["ELV", "CVS", "HUM", "CI", "XLV"],
    "ABBV": ["JNJ", "MRK", "PFE", "LLY", "XLV"],
    # Industrials
    "CAT":  ["DE", "CMI", "PCAR", "XLI"],
    "BA":   ["LMT", "RTX", "GD", "NOC", "XLI"],
    "GE":   ["RTX", "HON", "MMM", "XLI"],
    # Media / consumer cyclical
    "DIS":  ["NFLX", "CMCSA", "WBD", "XLC", "XLY"],
    "F":    ["GM", "STLA", "TSLA", "XLY"],
    # Telecom
    "T":    ["VZ", "TMUS", "XLC"],
    "VZ":   ["T", "TMUS", "XLC"],
    # ETFs (compared to their broader benchmark)
    "SPY":  ["QQQ", "IWM"],
    "QQQ":  ["SPY", "IWM", "XLK"],
    "IWM":  ["SPY", "QQQ"],
    "XLF":  ["SPY", "KBE", "KRE"],
...
# Default peer if a ticker isn't in the map — fall back to broad market
DEFAULT_PEER_FALLBACK = ["SPY"]
...
class PaperTrade:
    ...
    # Snapshot of all indicator readings at entry time — for win/loss analysis.
    # Stored as a flat dict so it survives JSON round-trip.
...
@st.cache_data(ttl=300)
def get_peer_divergence(ticker: str, lookback: int = 5) -> dict | None:
    """
    Compare a stock's recent return to its peers and sector ETF.

    Returns a dict with:
      - peers: list of peer tickers used
      - own_return_pct: stock's N-day return
      - peer_median_return_pct: median return across peers
      - divergence_pct: stock's return MINUS peer median (negative = lagging)
      - peer_returns: dict of {peer_ticker: return_pct} for display
      - signal: "ALONE_DOWN" | "ALONE_UP" | "WITH_PEERS" | "WEAKER" | "STRONGER" | "ALIGNED"

    "ALONE_DOWN": stock is down significantly while peers are flat/up
                  → idiosyncratic problem, AVOID bull put
    "ALONE_UP":   stock is up significantly while peers are flat/down
                  → idiosyncratic strength, AVOID bear call
    "WEAKER":     stock down with peers but more than them → mild caution
    "STRONGER":   stock up with peers but more than them → mild caution
    "WITH_PEERS": stock moving in same direction with similar magnitude
    "ALIGNED":    everyone roughly flat
    """
    peers = PEERS.get(ticker, DEFAULT_PEER_FALLBACK)
    if not peers:
        return None

    # Get own return
    own_hist = fetch_history(ticker, days=max(30, lookback * 4))
    if own_hist is None or len(own_hist) < lookback + 1:
        return None
    own_close = own_hist["Close"].dropna()
    if len(own_close) < lookback + 1:
        return None
    own_return = (own_close.iloc[-1] / own_close.iloc[-(lookback + 1)] - 1) * 100

    # Get peer returns
    peer_returns = {}
    for p in peers:
        ph = fetch_history(p, days=max(30, lookback * 4))
        if ph is None or len(ph) < lookback + 1:
            continue
        pc = ph["Close"].dropna()
        if len(pc) < lookback + 1:
            continue
        peer_returns[p] = float((pc.iloc[-1] / pc.iloc[-(lookback + 1)] - 1) * 100)

    if not peer_returns:
        return None

    peer_median = float(np.median(list(peer_returns.values())))
    divergence = float(own_return - peer_median)

    # Classify the situation
    # Thresholds: a 2% divergence over 5 days is meaningful for liquid large-caps
    same_direction = (own_return * peer_median) > 0
    own_down = own_return < -1.0
    own_up = own_return > 1.0
    peers_flat = abs(peer_median) < 0.75

    if own_down and peers_flat:
        signal = "ALONE_DOWN"
    elif own_up and peers_flat:
        signal = "ALONE_UP"
    elif own_down and peer_median > 0:
        signal = "ALONE_DOWN"  # peers actively rising while this falls — even worse
    elif own_up and peer_median < 0:
        signal = "ALONE_UP"
    elif same_direction and abs(divergence) < 1.0:
        signal = "WITH_PEERS"
    elif same_direction and divergence < -1.0:
        signal = "WEAKER"  # both down, but stock leading the decline
    elif same_direction and divergence > 1.0:
        signal = "STRONGER"  # both up, stock leading the rally
    else:
        signal = "ALIGNED"

    return {
        "peers": peers,
        "own_return_pct": float(own_return),
...
def analyze_ticker(ticker: str) -> dict | None:
    ...
    # Peer divergence — best-effort, may return None for tickers without peer mapping
    div = get_peer_divergence(ticker, lookback=5)
    ...
    if div is not None:
        out["Peer_Divergence_%"] = div["divergence_pct"]
        out["Peer_Signal"] = div["signal"]
        out["Own_5d_Return_%"] = div["own_return_pct"]
        out["Peer_Median_5d_%"] = div["peer_median_return_pct"]
        out["Peer_Detail"] = div  # full detail for the expander view
    else:
        out["Peer_Divergence_%"] = float("nan")
        out["Peer_Signal"] = "N/A"
        out["Own_5d_Return_%"] = float("nan")
        out["Peer_Median_5d_%"] = float("nan")
        out["Peer_Detail"] = None

    return out


# ---------------------------------------------------------------------------
# Ticker-specific learned rules (from backtest discovery)
# ---------------------------------------------------------------------------
# Import learned ticker-specific rules if a ticker_rules.py file is present
# alongside app.py. Generated by running backtest.py with --discover.
TICKER_RULES: dict = {}
try:
    from ticker_rules import TICKER_RULES as _LOADED_RULES  # type: ignore
    TICKER_RULES = _LOADED_RULES
except ImportError:
    pass


def _apply_ticker_rules(ticker: str, signal: str, row: dict,
                         score: int, reasons: list[str]) -> tuple[int, list[str]]:
    """
    Apply ticker-specific learned rules from backtest discovery.

    For each indicator that the backtest identified as predictive for this
    ticker, add a bonus if current value is in the historically-best range,
    or a penalty if outside. Bonus size scales with the discovered edge.

    No-op for tickers without rules in TICKER_RULES.
    """
    if ticker not in TICKER_RULES:
        return score, reasons

    direction_key = "BULL_PUT" if "BULL" in signal else (
        "BEAR_CALL" if "BEAR" in signal else None
    )
    if direction_key is None:
        return score, reasons

    rules = TICKER_RULES[ticker].get(direction_key, {})
    if not rules:
        return score, reasons

    # Backtest indicator name → scanner column name
    name_map = {
        "rsi": "RSI14",
        "mfi": "MFI",
        "zscore": "ZScore",
        ...
        "vwap_dist": "VWAP_Dist_%",
        "hv_rank": "HV_Rank",
        "dist_support_pct": "Dist_Support_%",
        "dist_resistance_pct": "Dist_Resistance_%",
    }
    ...
    for ind_name, info in rules.items():
        col = name_map.get(ind_name)
        if col is None:
            continue
        val = row.get(col)
        ...
        edge = abs(info.get("edge", 0))
        bonus_size = max(5, min(15, int(edge) // 2 or 5))

        if info["min"] <= val <= info["max"]:
            bonus_total += bonus_size
            in_range_count += 1
            reasons.append(
                f"🎯 {ticker}-pattern: {ind_name}={val:.2f} in optimal range "
                f"[{info['min']:.2f}, {info['max']:.2f}] (hist WR {info['win_rate']:.0f}%)"
            )
        else:
            bonus_total -= max(3, bonus_size // 2)
            out_range_count += 1

    if in_range_count >= 2 and out_range_count == 0:
        bonus_total += 10
        reasons.append(f"✅ {ticker}: all learned rules aligned — high conviction")

    if bonus_total != 0:
        reasons.append(
            f"({ticker} ticker-specific adjustment: "
            f"{bonus_total:+d} | {in_range_count} in range, {out_range_count} out)"
        )

    return score + bonus_total, reasons
...
def classify(row: dict) -> tuple[str, int, list[str]]:
    ...
    # ------ PEER DIVERGENCE ADJUSTMENTS ------
    # Mirror-stock comparison: is this stock moving with its sector, or alone?
    # Idiosyncratic moves (alone-down for bull put, alone-up for bear call) are dangerous
    # because they suggest stock-specific news rather than sector mean-reversion.
    peer_signal = row.get("Peer_Signal", "N/A")
    div_pct = row.get("Peer_Divergence_%", float("nan"))

    if peer_signal == "ALONE_DOWN":
        # Stock dropping while peers flat/up — likely idiosyncratic problem
        bull -= 25
        bull_r.append(f"⚠️ Stock weak vs peers ({div_pct:+.1f}% divergence) — idiosyncratic risk")
    elif peer_signal == "WEAKER":
        # Stock + peers both down, but stock falling faster
        bull -= 10
        bull_r.append(f"Stock leading sector decline ({div_pct:+.1f}% vs peers)")
    elif peer_signal == "WITH_PEERS" and not np.isnan(div_pct):
        # Sector-wide pullback — actually GOOD for bull put (mean reversion likely)
        bull += 8
        bull_r.append(f"Moving with sector ({div_pct:+.1f}% divergence)")

    if peer_signal == "ALONE_UP":
        # Stock pumping while peers flat/down — could be takeover/news, dangerous to short
        bear -= 25
        bear_r.append(f"⚠️ Stock strong vs peers ({div_pct:+.1f}% divergence) — idiosyncratic strength")
    elif peer_signal == "STRONGER":
        # Both up, stock leading the rally
        bear -= 10
        bear_r.append(f"Stock leading sector rally ({div_pct:+.1f}% vs peers)")
    elif peer_signal == "WITH_PEERS" and not np.isnan(div_pct) and div_pct > 0:
        # Sector-wide rally — okay setup for bear call
        bear += 5
        bear_r.append(f"Moving with sector ({div_pct:+.1f}% divergence)")

    # Apply ticker-specific learned rules (if a ticker_rules.py exists for this ticker)
    ticker = row.get("Ticker", "")

    if bull >= 60 and bull > bear:
        bull, bull_r = _apply_ticker_rules(ticker, "BULL_PUT", row, bull, bull_r)
        return "BULL PUT", bull, bull_r
    if bear >= 50 and bear > bull:
        bear, bear_r = _apply_ticker_rules(ticker, "BEAR_CALL", row, bear, bear_r)
        return "BEAR CALL", bear, bear_r
    if bull >= 35:
        bull, bull_r = _apply_ticker_rules(ticker, "BULL_PUT", row, bull, bull_r)
        # Re-check actionable threshold after the bonus may have lifted it
        if bull >= 60: return "BULL PUT", bull, bull_r
        return "WATCH (bull)", bull, bull_r
    if bear >= 30:
        bear, bear_r = _apply_ticker_rules(ticker, "BEAR_CALL", row, bear, bear_r)
        if bear >= 50: return "BEAR CALL", bear, bear_r
        return "WATCH (bear)", bear, bear_r
...
tab_scan, tab_open, tab_track, tab_history, tab_analysis, tab_settings = st.tabs(
    ["🔍 Scanner", "➕ Open Trade", "📊 Open Trades", "📋 History",
     "🔬 Win/Loss Analysis", "⚙️ Settings"]
)
...
with tab_scan:
    ...
    with at2:
        auto_track_threshold = st.number_input(
            "Threshold", min_value=50, max_value=200, value=85, step=5,
            disabled=not auto_track_enabled,
            help="Minimum score to auto-track. Default 85 means only highest-conviction "
                 "setups get auto-tracked, which makes win/loss analysis cleaner.",
        )
    with at3:
        ...
        if not rows:
            ...
            if auto_track_enabled and not actionable.empty:
                ...
                if not high_scores.empty:
                    ...
                    for _, hr in high_scores.iterrows():
                        ...
                        # Capture full indicator snapshot for later win/loss analysis
                        # (only scalar values that JSON-serialize cleanly)
                        snapshot = {}
                        for k in [
                            "RSI14", "MFI", "ZScore", "ADX", "VWAP_Dist_%",
                            "Volume_Ratio", "HV_Rank", "Sigma",
                            "Dist_Support_%", "Dist_Resistance_%",
                            "Above_200SMA", "Above_50SMA",
                            "Down_Streak", "Up_Streak",
                            "Peer_Divergence_%", "Peer_Signal",
                            "Own_5d_Return_%", "Peer_Median_5d_%",
                            "POP_%", "ROC_%",
                        ]:
                            v = hr.get(k)
            ...
            if not actionable.empty:
                ...
                show_cols = ["Ticker", "Signal", "Score", "Price", "RSI14", "MFI",
                             "ZScore", "ADX", "VWAP_Dist_%", "Volume_Ratio",
                             "Peer_Divergence_%", "Peer_Signal", "HV_Rank",
                ...
                for _, r in actionable.iterrows():
                    with st.expander(
                        ...
                        # ---- PEER MIRROR ANALYSIS ----
                        peer_detail = r.get("Peer_Detail")
                        if peer_detail:
                            st.markdown("---")
                            st.markdown("**🪞 Mirror Stocks (Peer Comparison)**")

                            sig = peer_detail["signal"]
                            div_pct = peer_detail["divergence_pct"]
                            own_ret = peer_detail["own_return_pct"]
                            peer_med = peer_detail["peer_median_return_pct"]
                            lookback = peer_detail["lookback_days"]

                            # Headline divergence verdict
                            if sig == "ALONE_DOWN":
                                st.error(
                                    f"⚠️ **{r['Ticker']} dropping alone** — down {own_ret:+.2f}% over "
                                    f"{lookback} days while peers median is {peer_med:+.2f}%. "
                                    f"Divergence: {div_pct:+.2f}%. Possible idiosyncratic problem "
                                    f"(earnings warning, downgrade, lawsuit). Bull put riskier than usual."
                                )
                            elif sig == "ALONE_UP":
                                st.error(
                                    f"⚠️ **{r['Ticker']} rallying alone** — up {own_ret:+.2f}% over "
                                    f"{lookback} days while peers median is {peer_med:+.2f}%. "
                                    f"Divergence: {div_pct:+.2f}%. Possible idiosyncratic catalyst "
                                    f"(takeover rumor, beat-and-raise). Bear call riskier than usual."
                                )
                            elif sig == "WEAKER":
                                st.warning(
                                    f"📉 Stock leading sector decline ({own_ret:+.2f}% vs peer "
                                    f"median {peer_med:+.2f}%, divergence {div_pct:+.2f}%)"
                                )
                            elif sig == "STRONGER":
                                st.warning(
                                    f"📈 Stock leading sector rally ({own_ret:+.2f}% vs peer "
                                    f"median {peer_med:+.2f}%, divergence {div_pct:+.2f}%)"
                                )
                            elif sig == "WITH_PEERS":
                                st.success(
                                    f"✅ Moving with sector — {own_ret:+.2f}% vs peer median "
                                    f"{peer_med:+.2f}% (divergence only {div_pct:+.2f}%). "
                                    f"Sector-wide move = mean reversion candidate."
                                )
                            else:
                                st.info(
                                    f"Sector aligned — {own_ret:+.2f}% vs peer median {peer_med:+.2f}%"
                                )
                            ...
                            peer_rows = [
                                {
                                    "Ticker": r["Ticker"] + " ★",
                                    f"{lookback}d Return": f"{own_ret:+.2f}%",
                                    "vs Peer Median": f"{div_pct:+.2f}%",
                                }
                            ]
                            for pt, pr in sorted(
                                peer_detail["peer_returns"].items(),
                                key=lambda x: x[1], reverse=True,
                            ):
                                peer_rows.append({
                                    "Ticker": pt,
                                    f"{lookback}d Return": f"{pr:+.2f}%",
                                    "vs Peer Median": f"{pr - peer_med:+.2f}%",
                                })
                            st.dataframe(
    ...
    if not closed:
        ...
        if view_filter == "Side-by-side comparison":
            ...
            if len(auto_trades) >= 5:
                ...
                buckets = [(85, 89), (90, 94), (95, 99), (100, 200)]
                bucket_rows = []
...
# ---------------------------------------------------------------------------
# TAB 5: Win/Loss Analysis
# Compare entry-time indicators between winners and losers to identify
# which signals actually have edge.
# ---------------------------------------------------------------------------
with tab_analysis:
    st.subheader("🔬 Win/Loss Analysis — what's actually working?")
    st.caption("Compares the indicator readings AT ENTRY for winning vs losing trades. "
               "Reveals which signals genuinely predict profitable setups vs which are noise.")

    trades = load_trades()
    closed_with_metrics = [
        t for t in trades
        if t.status == "CLOSED" and t.entry_metrics  # need a snapshot to analyze
    ]

    if not closed_with_metrics:
        st.info(
            "No closed trades with captured indicator data yet.\n\n"
            "**How this builds up:** Run the scanner with auto-track enabled. "
            "Every auto-tracked trade saves a snapshot of all indicator values "
            "at entry. Once those trades close (winners or losers), they show "
            "up here for analysis.\n\n"
            "**Note:** Manual trades don't have entry snapshots — only auto-tracked "
            "trades carry the data needed for this analysis. Scan often, let "
            "auto-tracker do its thing, and check back after 20-30 closed trades."
        )
    else:
        # Source filter — focus on AUTO since they have the snapshot
        source_filter = st.radio(
            "Trade source",
            ["🤖 Auto-tracked only", "All trades with metrics"],
            horizontal=True,
        )
        if source_filter == "🤖 Auto-tracked only":
            analysis_set = [t for t in closed_with_metrics if t.source == "AUTO"]
        else:
            analysis_set = closed_with_metrics

        if not analysis_set:
            st.info("No trades match the selected filter.")
        else:
            # Split winners from losers
            winners = [t for t in analysis_set if t.realized_pnl > 0]
            losers = [t for t in analysis_set if t.realized_pnl < 0]
            breakeven = [t for t in analysis_set if t.realized_pnl == 0]

            # Top metrics
            mc1, mc2, mc3, mc4 = st.columns(4)
            mc1.metric("Total trades analyzed", len(analysis_set))
            mc2.metric("✅ Winners", len(winners), f"{len(winners)/len(analysis_set)*100:.0f}%")
            mc3.metric("❌ Losers", len(losers), f"{len(losers)/len(analysis_set)*100:.0f}%")
            mc4.metric("➖ Breakeven", len(breakeven))

            # ---- METRIC COMPARISON ----
            # For each indicator, compute mean for winners and losers, plus the gap.
            # A meaningful gap = the indicator actually predicts outcomes.
            st.markdown("---")
            st.markdown("### 📊 Indicator Comparison: Winners vs Losers")
            st.caption(
                "For each indicator, this shows the average value at entry "
                "across winning trades vs losing trades. A large gap suggests "
                "the indicator has predictive power."
            )

            # Collect all numeric metric keys across the dataset
            all_keys = set()
            for t in analysis_set:
                for k, v in t.entry_metrics.items():
                    if isinstance(v, (int, float)) and not isinstance(v, bool):
                        all_keys.add(k)

            # Also include the score itself for comparison
            comparison_rows = []
            score_w = [t.scanner_score for t in winners if t.scanner_score]
            score_l = [t.scanner_score for t in losers if t.scanner_score]
            if score_w or score_l:
                comparison_rows.append({
                    "Indicator": "Scanner Score",
                    "Winner Avg": round(float(np.mean(score_w)), 2) if score_w else None,
                    "Loser Avg": round(float(np.mean(score_l)), 2) if score_l else None,
                    "Gap": (
                        round(float(np.mean(score_w) - np.mean(score_l)), 2)
                        if score_w and score_l else None
                    ),
                    "Winner N": len(score_w),
                    "Loser N": len(score_l),
                })

            for k in sorted(all_keys):
                w_vals = [t.entry_metrics[k] for t in winners if k in t.entry_metrics]
                l_vals = [t.entry_metrics[k] for t in losers if k in t.entry_metrics]
                if not w_vals and not l_vals:
                    continue
                w_avg = float(np.mean(w_vals)) if w_vals else None
                l_avg = float(np.mean(l_vals)) if l_vals else None
                gap = (w_avg - l_avg) if (w_avg is not None and l_avg is not None) else None
                comparison_rows.append({
                    "Indicator": k,
                    "Winner Avg": round(w_avg, 2) if w_avg is not None else None,
                    "Loser Avg": round(l_avg, 2) if l_avg is not None else None,
            ...
            if comparison_rows:
                # Sort by absolute gap to surface the most predictive indicators first
                comparison_rows.sort(
                    key=lambda r: abs(r["Gap"]) if r["Gap"] is not None else -1,
                    reverse=True,
                )
                st.dataframe(
                ...
                st.caption(
                    "Larger absolute gaps = more predictive indicators. "
                    "Be cautious of conclusions until you have at least 20+ trades "
                    "in each bucket — small samples can show fake patterns."
                )
            ...
            if not winners:
                ...
                with st.expander(f"View {len(winners)} winning trades", expanded=False):
                    for t in sorted(winners, key=lambda x: x.realized_pnl, reverse=True):
                        em = t.entry_metrics
                        with st.container():
                            st.markdown(
                                f"**{t.ticker}** {t.spread_type} • "
                                f"P&L: **${t.realized_pnl:+.2f}** • "
                                f"Score: {t.scanner_score} • "
                                f"Closed: {t.close_date} ({t.close_reason})"
                            )
                            st.caption(f"Entry {t.entry_date} @ ${t.entry_price:.2f}, "
                                       f"strikes {t.short_strike:.0f}/{t.long_strike:.0f}, "
                                       f"credit ${t.entry_credit:.2f}")
                            if em:
                                # Inline indicator panel
                                cols = st.columns(6)
                                metrics_to_show = [
                                    ("RSI14", "RSI"),
                                    ("MFI", "MFI"),
                                ...
                                for i, (key, label) in enumerate(metrics_to_show):
                                    if key in em and em[key] is not None:
                                        ...
                                        if isinstance(v, float):
                                            cols[i].metric(label, f"{v:.1f}")
                                        else:
                                            cols[i].metric(label, str(v))
                            st.markdown("---")

            # ---- LOSERS GROUP ----
            st.markdown("### ❌ Losing Trades")
            if not losers:
                st.info("No losing trades yet — but small samples can be misleading. "
                        "Wait until you have 20+ closed trades before drawing conclusions.")
            else:
                with st.expander(f"View {len(losers)} losing trades", expanded=False):
                    for t in sorted(losers, key=lambda x: x.realized_pnl):
                        em = t.entry_metrics
                        with st.container():
                            st.markdown(
                                f"**{t.ticker}** {t.spread_type} • "
            ...
            # ---- ACTIONABLE INSIGHTS ----
            if winners and losers and len(analysis_set) >= 10:
                st.markdown("### 💡 Insights from your data")
                insights = []
                for row in comparison_rows[:5]:  # top 5 most-divergent indicators
                    if row["Gap"] is None or abs(row["Gap"]) < 0.5:
                        continue
                    direction = "higher" if row["Gap"] > 0 else "lower"
                    insights.append(
                        f"- **{row['Indicator']}**: winners averaged "
                        f"{row['Winner Avg']:.2f} vs losers {row['Loser Avg']:.2f} "
                        f"(gap: {row['Gap']:+.2f}). Winning trades had "
                        f"{direction} {row['Indicator']} at entry."
                    )
                if insights:
                    st.markdown("\n".join(insights))
                    st.caption(
                        "**How to use this:** If a meaningful gap holds up over 30+ trades, "
                        "consider raising the score weight for that indicator (more aggressive "
                        "filtering) or making it a hard filter. If gaps are tiny across "
                        "everything, the strategy may have inherent noise that no indicator "
                        "fixes — focus on stricter score thresholds or smaller position sizing."
                    )
                else:
                    st.info(
                        "No indicator shows a meaningful gap between winners and losers in "
                        "your current sample. Either you don't have enough trades yet, or "
                        "your win/loss distinction is driven by factors not in the indicator "
                        "set (random variance, market regime changes, gap risk)."
                    )
            ...
            for t in analysis_set:
                row = {
                    ...
                    "outcome": "WIN" if t.realized_pnl > 0 else ("LOSS" if t.realized_pnl < 0 else "BE"),
                ...
                row.update(t.entry_metrics)
                export_rows.append(row)
            if export_rows:
                export_df = pd.DataFrame(export_rows)
                st.download_button(
                    "Download win/loss analysis CSV",
                    export_df.to_csv(index=False).encode(),
                    file_name=f"winloss_analysis_{datetime.now().strftime('%Y%m%d')}.csv",
                    mime="text/csv",
                )
...
version history of your trades.
    ...
    # Show ticker-specific learned rules status
    st.markdown("---")
    st.markdown("### 🎯 Ticker-Specific Learned Rules")
    if TICKER_RULES:
        st.success(f"Loaded learned rules for {len(TICKER_RULES)} ticker(s): "
                   f"**{', '.join(TICKER_RULES.keys())}**")
        with st.expander("View loaded rules"):
            for ticker, rule_set in TICKER_RULES.items():
                st.markdown(f"**{ticker}**")
                for direction, rules in rule_set.items():
                    if not rules:
                        continue
                    st.markdown(f"  *{direction}*")
                    rule_rows = []
                    for ind, info in rules.items():
                        rule_rows.append({
                            "Indicator": ind,
                            "Optimal min": f"{info['min']:.2f}",
                            "Optimal max": f"{info['max']:.2f}",
                            "Hist Win Rate": f"{info['win_rate']:.0f}%",
                            "Avg P&L": f"${info['avg_pnl']:+.0f}",
                            "Sample N": info['n'],
                            "Edge": f"${info['edge']:+.0f}",
                        })
                    if rule_rows:
        ...
        st.info(
            "No ticker-specific rules loaded. To learn rules for a specific ticker:\n\n"
            "1. Run `python backtest.py --ticker BAC --discover` (or any ticker)\n"
            "2. The script will write a `ticker_rules.py` file with learned thresholds\n"
            "3. Place that file alongside `app.py` and restart the app\n"
            "4. The scanner will apply those rules only to that specific ticker"
        )
